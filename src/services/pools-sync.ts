 ...
 *       deleted_origins and is never created again. Saves spend the pools-sync share of the osu!
 *       budget on their stats. Every write marks the pack's page and every public list stale.
...
/** Updates the pack when the input changed; answers unchanged, with no write, otherwise. */
const applyTo = async (
  doc: PackRecord,
  ownerId: string,
  input: PackInput,
): Promise<PoolsSyncAnswer> => {
  ...
  if (!pack) throw new Error(`pools pack ${current.slug} isn't the pools account's`);
  touch(pack.slug);
 ...
 *          whether /packs lists it; null when a moderator deleted this pool's pack
...
export const syncPoolsPack = async (
  ...
  if (await (await tombstones()).findOne({ _id: ref })) return null;
  const ownerId = await ensurePoolsAccount();
  const existing = await lookup(ref);
  if (existing) return applyTo(existing, ownerId, input);
  try {
    ...
    return applyTo(winner, ownerId, input);
  }
};
