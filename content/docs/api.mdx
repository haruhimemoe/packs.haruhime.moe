...
- `slots` holds beatmap (difficulty) IDs by slot. Titles, star ratings, and other beatmap details aren't included. Look them up on osu! or a beatmap mirror.
...
To list public packs (slug, name, owner, map count) without a key, use the static search index at [/packs/index.json](https://packs.haruhime.moe/packs/index.json) (up to 5,000 newest). It doesn't count toward any limit. For a pack's maps, call `GET /api/v1/packs/{slug}`.
