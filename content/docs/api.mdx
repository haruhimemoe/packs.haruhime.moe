...
- `complete`: `false` when we couldn't look up a map or a rating with mods. The numbers then cover the maps we could, and we try again later. A range is `null` when no map gave a value.
...
- 2026-09-24: pack objects carry `stats`, and the search index carries them in short form.
