...
Counters start over at the top of each minute (each hour for new keys). Every response from an endpoint that needs a key carries:
...
Map usage needs no key, and our CDN can answer it from its cache, so its answers are different: they carry `Cache-Control` and no rate-limit headers. Over their limit you still get the `429` below, with all four headers.
...
- `400` `bad_request`: the body, `page`, or a beatmap ID (in the path or in `ids`) isn't valid. The message says what to fix.
...
- `ownerName` is the owner's osu! username: `haruhime archive` on archive packs, or `Unknown player` when we don't have one.
...
- `stats` sums up the pack's maps. We work it out a few seconds after each save, so it's missing from the answer to `POST` and to a `PUT` that changes the maps or slots; read the pack again a little later. It's also missing for a pack whose stats we haven't worked out yet. Archive packs start with numbers from otdb instead: see `complete`.
...
- `complete`: `false` when we couldn't look up a map or a rating with mods. The numbers then cover the maps we could, and we try again later. A map osu! says doesn't exist (deleted, say) is left out of the numbers and doesn't make `complete` false. A range is `null` when no map gave a value. Archive packs start with `complete` false and otdb's numbers (star ratings without mods, length and BPM), even when otdb had every map. A daily job then works their stats out like any other pack's, archive packs last, so that can take many days.
- `computedAt`: when we worked the stats out, or, for otdb's numbers, when we imported the pool.
...
- `fingerprint`: the pool's identity, a sha256 of its sorted `beatmapId:mods` entries. The same pool from two sources is one pack with two sources. A pool that changed maps at its source is a new pack, and the old pack becomes unlisted once every source of it changed: its link still works, but it leaves the public list and map usage.
- `sources`: where the pool came from, first import first: `kind` (`otdb`, `otr` or `wybin`; every pool so far comes from `otdb`), the pool's `id` there, its `url`, and `importedAt`.

## Map usage

Map usage says which archive packs used a map, so you can see how often and how recently it was pooled. Only the archive packs listed on [/packs](https://packs.haruhime.moe/packs) count (public and not hidden); community packs don't.
  ...
  "entries": [
    {
      ...
      "slot": "NM1",
      "mods": "NM",
      "fingerprint": "7efc8272a32f53fd013e43f9a2fc5be0422fa65bd1aaee14250ba68cbe504160"
    },
    {
      "slug": "Uakgb_J5m9",
      ...
      "slot": "HDHR2",
      "mods": "HDHR",
      "fingerprint": "622a494d3ea8c7ba2fed4f37909f14d9b50ab412322de39be62c8d6c2418bfca"
    }
  ]
}
```
...
- `fingerprint`: the pool's fingerprint, the same as its pack's `archive.fingerprint`. Pools with the same maps and mods share it, so to show a pool's maps without counting that pool, leave out the entries whose `fingerprint` is the pool's own.

A map no archive pack used comes back with a `count` of 0 and no entries, never a 404. Usage changes when we import pools or a moderator hides, unhides or deletes one; answers can be up to an hour old.
...
Public packs, most recently updated first, 50 per page, archive packs included. Takes `?page=` (see Pagination). Each entry in `packs` is a full pack object, shortened here. Pinned packs aren't marked or moved up here; the Pinned row is only on the site.
