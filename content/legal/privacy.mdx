...
- **Requests to our pools service without its token.** Only pools.haruhime.moe has the token for the endpoint it publishes packs through. A request to it without the token is counted per IP address for about 2 minutes, to stop token guessing.
