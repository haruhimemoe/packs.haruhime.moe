...
- **Map usage lookups.** Pack pages and the editor ask our server which archived tournament pools used each map (`/api/v1/beatmaps/usage`), signed in or not, and anyone can ask the API the same without a key. Our server counts these requests per IP address in short-lived counters, deleted automatically within about two minutes. The answers hold nothing about you.
