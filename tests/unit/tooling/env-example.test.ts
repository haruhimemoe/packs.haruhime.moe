 ...
 * @desc .env.example documents every server variable (CRON_SECRET too) and ships no secret
 *       values.
...
import { CRON_SECRET_KEY, SERVER_ENV_KEYS } from "@/env";

const text = readFileSync(path.join(process.cwd(), ".env.example"), "utf8");

describe(".env.example", () => {
  it.each([...SERVER_ENV_KEYS, CRON_SECRET_KEY])("documents %s", (key) => {
